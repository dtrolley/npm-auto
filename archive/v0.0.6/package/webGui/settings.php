...
function require_auth() {
  ...
  if ($admin_token) {
    if (!$hc || $hc !== $admin_token) { http_response_code(403); echo json_encode(['ok'=>false,'error'=>'auth']); exit; }
  } else {
...
if ($action === 'rescan') {
  require_auth();
  // simple rescan: return list of running containers
  $containers = array();
  exec('docker ps --format "{{.Names}}"', $containers);
  echo json_encode(['ok'=>true,'containers'=>$containers]);
  exit;
}
if ($action === 'createAll') {
  require_auth();
  // in this simplified implementation we just report which would be created
  $containers = array();
  exec('docker ps --format "{{.Names}}"', $containers);
  $report = array('will_create'=> $containers);
  echo json_encode(['ok'=>true,'report'=>$report]);
  exit;
}
if ($action === 'deleteOrphans') {
  require_auth();
  // placeholder: list currently configured domains in state that have no container running
  if (!file_exists($STATE)) file_put_contents($STATE, json_encode(new stdClass()));
  $s = json_decode(file_get_contents($STATE), true) ?? array();
  $running = array();
  exec('docker ps --format "{{.Names}}"', $running);
  $orphans = array();
  foreach ($s as $name=>$v) {
    if (!in_array($name, $running)) $orphans[] = $name;
  }
  echo json_encode(['ok'=>true,'orphans'=>$orphans]);
  exit;
}
